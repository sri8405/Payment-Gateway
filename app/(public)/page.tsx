      ...
      <section className="border-b bg-white">
        <div className="mx-auto grid max-w-6xl gap-8 px-4 py-12 md:grid-cols-[1.2fr_0.8fr] md:items-center">
          <div className="space-y-5">
            <p className="text-sm font-semibold uppercase tracking-wide text-accent-foreground">Seva Booking & Management</p>
            <h1 className="text-4xl font-bold leading-tight text-foreground sm:text-5xl">{settings.templeName}</h1>
            <p className="max-w-2xl text-lg text-muted-foreground">
              {settings.templeDescription || "Book seva online with a simple acknowledgment flow for devotees and a focused management console for temple administrators."}
            </p>
          ...
          <div className="rounded-lg border bg-secondary p-6">
            ...
            <p className="mt-2 text-sm text-muted-foreground">Available for online seva booking today</p>
          </div>
        </div>
      </section>
        ...
        <div className="grid gap-6 lg:grid-cols-[1.4fr_0.6fr]">
          ...
          {sevas.map((seva) => (
            <Card key={seva._id}>
              ...
              <CardContent>
                <p className="min-h-10 text-sm text-muted-foreground">{seva.description || "Book seva for this offering."}</p>
